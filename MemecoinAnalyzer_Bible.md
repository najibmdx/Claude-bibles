...
_Last updated: 2026-05-16 (Session 7 complete — YAE Case 005 + Type 2C candidate + Forensic Cleanliness vs Tradeability framework added)_
...
### Type 2C Candidate: Serial Operator + Creator Self-Allocation Dump `[HYPOTHESIS — n=1, YAE Case 005]`
...
| Type | Supply control mechanism | Dump wallet | YAE fit? |
|---|---|---|---|
| Type 2B Serial Operator + Bundle | Fresh/controlled wallet receives large block-0 allocation | Bundle receiver wallet | No |
| Type 2C Candidate | Dev wallet receives creator-side allocation and dumps directly | Dev wallet | Yes |
...
### The Creator Self-Allocation Dumper `[HYPOTHESIS — n=1, YAE Case 005]`
...
**Operation:** Type 2 Serial Operator + creator self-allocation dump `[HYPOTHESIS — n=1 candidate Type 2C]`
...
| `HiSo5kykqDPs3EG14Fk9QY4B5RvkuEs8oJTiqPX3EDAn` | SerialDev_02 / SelfAllocationDev_01 | YAE + 92 sampled `CreateV2` launches | YAE dev. Received 9.6449567% supply in creation tx and sold full allocation 2 seconds later. Candidate Type 2C pattern. Funding source/genesis unknown. |
...
| `DsCJ5siuJTPQtQa3A9N69azGZaWtUPzi9VPp2G9Jfpx9` | MEV / automated flow bot | HANTA re-pump; YAE live ATH expansion | HANTA: 55% fee payer during re-pump. YAE: recurring top fee payer during live run — 26/160 txs = 16.25% and 33/220 pool tx sample. Recurrence across 2 cases strengthens bot-watch priority. In YAE, demand still absorbed flow — do not treat as automatic exit trigger. |
...
The following showed fee-payer concentration in YAE samples but have no recurrence proof or identity proof yet. Do not label as bots until confirmed across cases.

| Wallet fragment | Seen On | Notes |
|--------|---------|-------|
...
_Note: Full addresses must be recovered before these entries can be promoted to the main bot registry._
...
15. **Does the creator self-allocation then instant sell pattern recur across `HiSo5kykqDPs3EG14Fk9QY4B5RvkuEs8oJTiqPX3EDAn` launches?** (Case 005 open)
...
### Next deliberate selections after Case 005

| Case | What to find | Framework question |
|------|-------------|-------------------|
| 006 | Another token by `HiSo5kykqDPs3EG14Fk9QY4B5RvkuEs8oJTiqPX3EDAn` | Does creator self-allocation dump recur across this dev's launches? |
| 007 | Another token with dev self-allocation >5% but no fresh bundle wallet | Is Type 2C real or YAE-only? |
| 008 | Another live dirty-but-tradeable pump | Can `DIRTY BUT TRADEABLE` be detected earlier before major run? |
| 009 | Token where `DsCJ5siuJTPQtQa3A9N69azGZaWtUPzi9VPp2G9Jfpx9` appears again | Does this bot recur across profitable ATH expansion phases? |
| 010 | Token where `BX7h4...` / `AxGt...` / `BaXmv...` recur | Confirm or reject YAE fee-payer candidates as bot infrastructure (full addresses needed) |
| 011 | Clean non-dormant organic pump | Establish the control sample against dirty-but-tradeable cases |
| 012 | Failed dirty launch with dev self-allocation dump | What separates YAE-style survival from immediate death? |
| 013 | Token where `4QfGpNuC5eeM9ieEp4qvFH49uWm4WqKTU4GQGvKZZyUc` or `6bZHJzbApPZck2gauwxmsEGkzPGZxxawoNuEFV1HtbDr` reappear | KOL network cross-token fingerprinting |
| 014 | Any token where `FireuLYd4yjJBhXQyBs3Mq6ZpNEjyHPNPG2vEMd3cxGS` or `DsCJ5siuJTPQtQa3A9N69azGZaWtUPzi9VPp2G9Jfpx9` reappear | Bot infrastructure cross-token confirmation |
| 015 | Any token where `7ofDdxo7mMzfK25ZbrMt8ErvnvSLUNXp2Xn2jHesryxK` appears | Confirm professional trader recurrence |
| 016 | Current live token pre-ATH | First real-time framework test |
